...
tf.app.flags.DEFINE_integer('embedding_dim', 300, 'dimension of word embedding') # 词嵌入维度：300
tf.app.flags.DEFINE_integer('batch_size', 25, 'number of example per batch') # batch_size ：25 个数据
tf.app.flags.DEFINE_integer('n_hidden', 300, 'number of hidden unit') # 隐藏h：300 个隐藏单元cell
tf.app.flags.DEFINE_float('learning_rate', 0.01, 'learning rate') # 学习率 ： 0.01
tf.app.flags.DEFINE_integer('n_class', 3, 'number of distinct class') # 分类数量： 3个
tf.app.flags.DEFINE_integer('max_sentence_len', 80, 'max number of tokens per sentence') # max_sentence_len：每个句子分割的最大长度
tf.app.flags.DEFINE_float('l2_reg', 0.001, 'l2 regularization') # l2_reg 规范化
tf.app.flags.DEFINE_integer('display_step', 4, 'number of test display step') # 测试显示步数：4
tf.app.flags.DEFINE_integer('n_iter', 8, 'number of train iter') # iter数量 ： 8
tf.app.flags.DEFINE_float('keep_prob1', 1.0, 'dropout keep prob') # dropout1 ： 1.0
tf.app.flags.DEFINE_float('keep_prob2', 1.0, 'dropout keep prob') # dropout2 ： 1.0


tf.app.flags.DEFINE_string('train_file_path', 'data/restaurant/rest_2014_lstm_train_new.txt', 'training file') # 训练数据路径
tf.app.flags.DEFINE_string('validate_file_path', 'data/restaurant/rest_2014_lstm_test_new.txt', 'validating file') # 验证数据路径
tf.app.flags.DEFINE_string('test_file_path', 'data/restaurant/rest_2014_lstm_test_new.txt', 'testing file') # 测试文件路径
tf.app.flags.DEFINE_string('embedding_file_path', 'data/restaurant/rest_2014_word_embedding_300_new.txt', 'embedding file') # 词嵌入的路径
tf.app.flags.DEFINE_string('word_id_file_path', 'data/restaurant/word_id_new.txt', 'word-id mapping file') # word->id 路径
tf.app.flags.DEFINE_string('aspect_id_file_path', 'data/restaurant/aspect_id_new.txt', 'word-id mapping file') # aspect -> id 路径
tf.app.flags.DEFINE_string('method', 'AT', 'model type: AE, AT or AEAT') # 模型类型
tf.app.flags.DEFINE_string('t', 'last', 'model type: ') # # 模型类型
...
class LSTM(object):
    ...
    def __init__(self, embedding_dim=100, batch_size=64, n_hidden=100, learning_rate=0.01,
        ...
        self.word_embedding = tf.Variable(self.w2v, dtype=tf.float32, name='word_embedding') # 定义word_embedding变量
        ...
        # dict(1219)            1220 * 300
        self.aspect_embedding = tf.Variable(self.aspect_embed, dtype=tf.float32, name='aspect_embedding') #定义word_embedding变量
